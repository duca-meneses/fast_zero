...
def test_list_todos_should__return_5_todos(session, client, user, token):
    expected_todos = 5
    session.bulk_save_objects(TodoFactory.create_batch(5, users_id=user.id))
    session.commit()
...
def test_list_todos_filter_title_should_return_5_todos(
    ...
    session.bulk_save_objects(
        TodoFactory.create_batch(5, users_id=user.id, title='Test todo 1')
    )
    session.commit()
...
def test_list_todos_filter_description_should_return_5_todos(
    ...
    session.bulk_save_objects(
        TodoFactory.create_batch(
            5, users_id=user.id, description='description'
        )
    )
    session.commit()
...
def test_list_todos_filter_state_should_return_5_todos(
    ...
    expected_todos = 5
    session.bulk_save_objects(
        TodoFactory.create_batch(5, users_id=user.id, state=TodoState.draft)
    )
    session.commit()
